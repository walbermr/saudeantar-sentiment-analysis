...
def post_process(*pred):
    output = torch.sigmoid(pred[0]).flatten()
    return output


def main():
    model_name = "checkpoint-4353"
    model_path = "./onnx-models/" + model_name + ".onnx"
    model = AutoModelForSequenceClassification.from_pretrained("./test_trainer/" + model_name)
    ...
    augmented_model = pnp.export(
        ...
        input_names=['input'],
        output_names=['output'],
        output_path="./onnx-models/" + model_name + '-aug.onnx',
        dynamic_axes={'input': [0], 'output': [0]}
    )

    test_input = ["bem macio e felpudo...recomendo.  preco imbativel e entrega rapida. compraria outro quando precisar"]

    # Load the model
    session_options = onnxruntime.SessionOptions()
    session_options.register_custom_ops_library(onnxruntime_extensions.get_library_path())
    session = onnxruntime.InferenceSession("./onnx-models/" + model_name + '-aug.onnx', session_options)

    # Run the model
    results = session.run(["output"], {"input": test_input})

    print(results)
    print("\nResult is: " + ("positive" if results[0] > 0.5 else "negative"))
